...
The XSO framework is an extension of Xarray-simlab \citep{Bovy2018Xarray-simlab:Interactively}, which provides a generic and highly flexible model development framework in Python. It relies on advanced Python functionalities, such as compact data classes and decorators (see the online-documentation for more details), to provide a relatively simple user interface to construct models, that is detached from a more complex backend code. In addition to this interface, Xarray-simlab provides powerful data handling capabilities, storing model input and output as labelled multidimensional Xarray datasets \citep{Hoyer2017Xarray:Python}. Model output as Xarray datasets are compatible with a wealth of other Python tools for data analysis or visualisation and can be directly exported to the NetCDF file standard (amongst others).

Xarray-simlab has found fruitful applications, for example in landscape evolution modeling \citep{benoit_bovy_2020_3840917} and plant growth modeling \citep{Vaillant2022TowardsDevelopment}. The Xarray-simlab framework is generic in that it provides only a step-wise execution of model processes and no concept of time. XSO is technically a wrapper around Xarray-simlab, adding custom building blocks and backend code to allow a user to easily define and compute models based on differential equation.
...
A dedicated open-source effort to simplify this step in particular, is the comprehensive Framework for Aquatic Biogeochemical Models (FABM) \citep{Bruggeman2014a}. FABM provides flexibility in the linking ecosystem models with physical models of varying dimensionality. The models implemented generally allow some modification via markup language files (YAML), but the general ecosystem model formulation are hard-coded in Fortran scripts.
