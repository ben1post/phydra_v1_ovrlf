...
\begin{comment}
- Introductory sentence, what is XSO? 
- Extends the Xarray-simlab framework to inherent ODE functionality
- Xarray extension, what can it do?
- 
\end{comment}

XSO (short for Xarray-simlab-ODE) is a Python package that provides a framework for building and running computational models based on ordinary differential equations (ODEs).

- ODE model extension of Xarray-simlab \citep{Bovy2018Xarray-simlab:Interactively}. Xarray-simlab provides a generic framework for building computational models in a modular fashion and an extension for storing model parameters, running simulations and storing output using the xarray.Dataset structure. XSO inherits most functionality from that package but extends it with a xarray-simlab process construcor (creates \textit{components}), new variable types and a model and solver backend. Interface is only wrapped not hidden, so allows many custom extensions for specific applications.

- XSO components built from XSO variable types, explained in detail below


-The model construction process is self-documenting, as parameters and model output, as well as user-supplied metadata is stored in a single Xarray dataset.
...
\begin{enumerate}
    ...
    - Custom attributes that can construct a \textit{component}: \textit{variable}, \textit{forcing}, \textit{parameter}, \textit{flux} or \textit{group}. Variable types are explained in greater detail in section \ref{Section:CreatingXSOComponent}.
    
    \item \textbf{\textit{Component}}: 
    - decorated Python class, that converts defined variable types into a fully functional model component
    - Decorator: \texttt{xso.component}
    
    \item \textbf{\textit{Model object}}: 
    - created with function \texttt{xso.model()}, supplying dictionary of model components with their labels
    
    An XSO model object includes backend processes handling the solver and model assembly, as well as processes defining state variables, forcings, parameters and their interactions (i.e. fluxes).  
    
    - Provide specific string labels for each component, allows re-use of components with specific labels.
    ...
    - Xarray.dataset
    - created with function \texttt{xso.setup()}, supplying solver, model object, input parameters and time steps that model output is returned in (for adaptive step-size solvers, e.g. odeint) or actual solver time steps (for step-wise solvers, e.g. GEKKO and stepwise)
    
    At model setup, the xarray-simlab framework supports batch dimensions of single or multiple parameters along which the model is solved for each set of parameters supplied. This provides an intuitive interface to explore the parameter space of models.
    
    The function call returns an Xarray dataset object that contains all provided parameters as well as the initialized labelled model dimensions. This dataset can be passed with the appropriate model object to be solved in the next step, but also provides methods to exchange parameters to easily modify an already instantiated model setup.
    
    \item \textbf{\textit{Model output}}: 
    - Xarray.dataset
    - returned when calling \texttt{model.xsimlab.run()} method on \textit{model setup} object.
...
% SOLVER
XSO currently provides three solving algorithms: Odeint, GEKKO and a simple stepwise solver. GEKKO is an open-source, object-oriented library of model construction, analysis and optimisation tools built on a core algebraic modelling language \citep{Beal2018GEKKOSuite}. For up to date information consult the online documentation (see code availability section).

XSO provides an interface for iterative modification, both to more complex and simpler model constructs and allows running parameter ranges as batch dimensions. The specific steps of model development are presented in further detail in the section \ref{Section:ModelDevelopmentWorkflow}.
...
\begin{enumerate}
    ...
    A typical model development workflow using Phydra would assemble a model instance from the basic building-blocks: XSO \textit{components} defining state variables, forcings and fluxes. 
    ...
    The model can be assembled from the Phydra library, modifications of provided base classes or custom written processes. 

    
    \item \textbf{Setting up the model}: 
    A XSO model object is itself only a collection of abstract \textit{components}. In order to fully initialize a model that can be solved, the model object needs to be supplied with a complete set of parameters, as well as a choice of parameters and the solving algorithm to use.
    This is done using the \texttt{xso.setup()} function. The necessary arguments to this function are the model object to be setup, all required input parameters, the time range to solve the model and the specific model output to save during runtime.
    ...
    - Xarray provides built-in functions for data analysis and visualization, easy to convert to Pandas and Numpy
    - rich interface and full compatibility with advanced data analysis packages
...
- This model development workflow is shown in full for the presented use cases in Jupyter notebooks in the model examples folder of Phydra.
...
\begin{itemize}
    ...
    State variables can be defined as unique instances, but also as functional groups (essentially an array of state variables), and Phydra provides functions capable of flexibly handling multi-dimensional interactions between model components.
    
    \item \textbf{\textit{forcing}}: Class attribute, requires setup function
    Defines external time-dependent variable in model
    
    orcings are defined as providing an external constant or time-varying parameter to the model. These parameters can be used in fluxes that reference the forcing type,
    ...
    Model parameter to be used in fluxes and setup functions
    ...
    Collects the value of variables defined in other \textit{components} with the matching group label. Currently this functionality only allows collecting the value of fluxes.
    
\end{itemize}

- To construct a component, the \textit{variable types} can be defined as attributes.
