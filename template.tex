...
(need to make clear that each of these is also a Xarray-simlab object, except components and variably types, which simply wraps underlying Xarray-simlab objects for ease of use)
\begin{itemize}
    \item \textbf{\textit{Variable types}}: 
    The most basic element of a model.
    
Declaring variables mainly consists of defining useful metadata such as:
...
user-defined meta-data attributes, e.g., units or math symbol,
    ...
    Logical component of the computational model that declares a subset of variables used in the model and defines a specific set of mathematical functions computed for these variables during model runtime.
...
\begin{enumerate}
    ...
    A \textit{model object} can be imported and used or assembled from the library of \textit{components}. 
