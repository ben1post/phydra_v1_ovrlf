...
\caption{Results of the NPZD model (application 2) for locations BIOTRANS, India, Papa, and KERFIX. We show here the final year of a five-year run. The data for nitrogen in the upper mixed layer (dots) are taken from WOA 2018. Phytoplankton nitrogen biomass (dots) is calculated using fixed ratios from the monthly chlorophyll climatology from MODIS Aqua satellite data. See the }
\label{Figure:ResultsEMPOWER}
\end{figure*}


Model outputs for all state variables in the corresponding stations are shown in Figure \ref{Figure:ResultsEMPOWER}.
...
--> Here discuss the change in light attnuation function! Refer all other points to Anderson since I didn't investigate any of it!
