...
- Efforts such as FABM-ERSEM provide 
- Model components written in FORTRAN are less accessible and flexible, modularity is relatively rough grain
- integrated, granular control --> stay in Python (tradeoff is there)
...
Xarray-simlab-ODE (XSO) is a Python framework that allows you to construct and customize models based on ordinary differential equations (ODEs) in a modular fashion. XSO was developed as the technical foundation of the Phydra library, but is not limited to any particular domain and can be used to create ODE-based models of any type.

The XSO framework is an extension of Xarray-simlab \citep{Bovy2018Xarray-simlab:Interactively}, which provides a generic and highly flexible model development framework in Python using compact data classes and decorators. It provides powerful data handling functionality, storing model input and output as Xarray datasets \citep{Hoyer2017Xarray:Python}. Xarray-simlab has found fruitful applications, for example in landscape evolution modeling \citep{benoit_bovy_2020_3840917} and plant growth modeling \citep{Vaillant2022TowardsDevelopment}. The Xarray-simlab framework is generic in that it provides only a step-wise execution of model processes and no concept of time. XSO wraps Xarray-simlab, adding custom building blocks and backend code to define and compute differential equation based models.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{xso.flux}: Defines a mathematical term with the \textit{variable types} in the \textit{component}, and adds the term into the system of differential equations of the underlying model.
        \item \texttt{xso.group}: Collects a group of state variables within the same model and with the same label in one component, for flexibly adding state variables from other \textit{components} to a \textit{flux}.
    \end{itemize}
