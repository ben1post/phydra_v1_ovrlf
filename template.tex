...
- phydra as a model library, for now only the 3 presented models, but in future many more can be included, or these presented models can be extended. All included components are theoretically compatible, so a model can be mixed-and-matched by users.


- For Use Case 1 we will show full model setup, including variables, passing parameters, etc.
- The others will be presented more schematically, without complete mentions of all variables, but for all models full model code, parameters, runs, code for plots etc. is available online for free
...
\begin{comment}
So now it is just Monod, which makes it much more straightforward to present! now I need to focus in the "implementation" part, to explain the schematic well, and make sure the model workflow is explained there as well..
...
- Also interesting for phytoplankton modelers: Very simple test-bed for more complex model setups (see Use Case 3)
- applicability in natural environments is limited, but can correspond to upwelling systems in the ocean.
...
- canonical formulation used in most large marine ecosystem models to this day
- note the caveats of using Monod, (steady state, bulk properties) -> however hear we want to present the simplest possible 
- first we check the model at constant flow rate and nutrient concentration, then (in order to show added value of framework) we show the simplicity of adding a different forcing, here sinusoidal varying nutrient concentration in supplied medium

In the following sections we will show the definition this models for growth of phytoplankton on a single nutrient in a chemostat environment.


\subsubsection{Description}
The model is schematized in Figure \ref{Figure:ModelSchematics_1} (a). It uses nitrogen as its currency (quantities are measured in units of $\mu mol$ N $m^{-3}$, or $\mu M$), with state variables for dissolved nutrients ($N$) and phytoplankton ($P$). The physical environment is a highly simplified flow-through system, corresponding to a laboratory chemostat setup. Growth medium with nutrient concentration $N_0$ flows into the system at rate $f$. The model components ($N$ \& $P$) flow out of the system at that same rate.
...
%%f
\begin{figure}[t]
\includegraphics[width=5cm]{Figures/firstdraft_schematics/code_schematics/XSOImplementations_2.pdf}
\caption{Schematic of the "SinusoidalForcing" XSO component, that can be exchanged for the "ConstantForcing" component at model creation. Labels and other aspects of the model implementation remain the same. The sinusoidal forcing requires additional input parameters, which have to be supplied by the user at model setup.}
\label{Figure:CodeSchematics_1b}
\end{figure}
...
%%f
\begin{figure*}[t]
\includegraphics[width=15cm]{Figures/firstdraft_schematics/code_schematics/XSOImplementations_3.pdf}
...
$k_N$ & half-sat. const: N uptake & 0.85 & \unit{µM \ N} \\
$m_P$ & linear P mortality & 0.015 & \unit{d^{−1}} \\
$m_{P2}$ & quadratic P mortality & 0.025 & \unit{(µM \ N)^{-1} d^{−1}} \\
$I_{max}$ & Z max. ingestion rate & 1.0 & \unit{d^{−1}} \\
$k_Z$ & Z half-saturation for intake & 0.6 & \unit{µM \ N} \\
...
$m_Z$ & linear Z mortality  & 0.02 & \unit{d^{−1}} \\
$m_{Z2}$ & quadratic Z mortality & 0.34 &\unit{(µM \ N)^{-1} d^{−1}}  \\
$v_D$ & D linear sinking rate &  6.43 & \unit{m \ d^{−1}}\\
$m_D$ & D remineralisation rate & 0.06 & \unit{d^{−1}} \\
$\kappa$ & constant mixing parameter & 0.13 & \unit{m \ d^{−1}}\\
$\kappa$ & constant mixing parameter & 0.13 & \unit{m \ d^{−1}} \\
...
%%f
\begin{figure*}[t]
\includegraphics[width=15cm]{Figures/firstdraft_schematics/code_schematics/XSOImplementations_4.pdf}
