...
\title{XSO framework (v0.1) and Phydra library (v0.1) for a flexible, reproducible and integrated plankton community modeling environment in Python} 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{xso.index}: Creates an input parameter to define a labelled dimension (i.e., Xarray index) within the model, stored as metadata in the model and output dataset.
    \end{itemize}
...
Chemostats are a commonly used experimental setup for studying the dynamics of microorganisms under controlled laboratory settings. They are characterized by a constant inflow of the medium containing nutrients and a constant outflow of the culture, both at a fixed rate $d$ (\unit{d^{-1}}). Under constant conditions, a steady-state emerges that is particularly useful for studying growth dynamics of microorganisms. Although the conditions of chemostat systems do not have a direct equivalent in nature, some oceanic upwelling systems can be approximated with such a simple model \citep{Haefner2005ModelingApplications}.
...
\bottomhline
\end{tabular}
\belowtable{Parameters adapted form \citet{Banas2011b}. Original sources for allometric functions: \citet{Tang1995TheRespiration, Eppley1969HalfSaturationPhytoplankton, Hansen1997ZooplanktonRange, Hansen1994a}} % Table Footnotes
...
\codeavailability{XSO and Phydra packages are open source and available under a BSD-3 license on GitHub.
- add actual links to Repos + Citation via Zenodo here - 
...
We want to thank the open-source developers, that made this work possible. In particular, we want to thank Benoît Bovy for writing Xarray-simlab, which this work is based upon, and initial advice on some aspects of the code structure.
