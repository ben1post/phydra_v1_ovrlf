...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{xso.variable}: Defines a state variable in the model, and can also be used to reference a state variable across the model.
        \item \texttt{xso.forcing}: Defines an external forcing as a constant value, or time-varying value, via an additional setup function.
        \item \texttt{xso.parameter}: Defines a constant model parameter.
        \item \texttt{xso.flux}: Defines a mathemtical term in the system of differential equations, utilizing the 
        \item \texttt{xso.group}:
    \end{itemize}These can be used to define variables in compact Python classes, to construct functional XSO \textit{components}. All of them can be defined with a variable number of dimensions.
