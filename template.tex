...
In the following use case we will show the definition of both of these models for growth of phytoplankton on a single nutrient in a chemostat environment. 
...
\caption{Model schematic of chemostat model. (a) Monod representation of phytoplankton feeding on a nutrient. (b) Droop representation of phytoplankton growth, including an added state variable for internal quota.}
\label{Figure:ModelSchematics_1}
\end{figure}

\subsubsection{Model formulation - Monod}
...
\subsubsection{Implementation - Monod}
...
- differences between Monod and Droop? 
-- Of course added state variable.. uptake and growth are decoupled, so uptake is much faster in Droop.. onset of growth is faster.
-- steady state biomass slightly lower, of course due to Qmin, always retain some biomass in internal pool, not in biomass
-- growth is faster, but shape of growth curve is very similar <- growth speed highly dependent on parameters!
-- in sinusoidal case, q shows dampened oscillations as it varies only slightly rises above Qmin
...
\caption{Model output for the four chemostat model scenarios. (a) Monod - constant, (b) Monod - sinusoidal, (c) Droop - constant, (d) Droop sinusoidal}
...
\subsubsection{Implementation / modification}
% here present the implementation as is used in phydra
% IDEA: instead of showing any code, show this also as a schematic (essentially the "map" of components and their variables) for the first simple example, I could show all steps of the process as well as the variables, etc. for the two more complex ones, I could isntead only show the name of the component in a box, and how they are interconnected (leaving out backend stuff)
...
technical stuff:
- however we modified the explicit time-step (in a for-loop) scheme to fully integrated functions, that can be computed irrespective of time step size. This makes the functions and code easier to read and modify.
