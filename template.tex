...
\title{The Phydra (v0.1) library and XSO (v0.1) framework for flexible, modular and reproducible marine ecosystem modeling based on differential equations in Python} 
...
In the following years this development received much scrutiny, positing \citet{Anderson2005} to ask if marine ecosystem was "running before we can walk?". The added state variables greatly increased the model parameter space, which lead to difficulties in validating model output in light of insufficient information \citep{Shimoda2016}.

- PFT models generally include very simplistic descriptions of ocean ecosystem and unavoidably limit the characterization of a diverse phytoplankton community.

A different approach was taken by trait-based models, that resolve an entire spectrum of one or more ecosystem component \citep{Bruggeman2007a, Merico2009, Follows2007EmergentOcean}. The difficulty of defining a complex parameter space was here simplified by constraining the spectrum to trade-offs between functional traits, e.g. between nutrient affinity and growth rate. 

and these models have already been applied in global three-dimensional model settings \citep{Dutkiewicz2020DimensionsDiversity}.

More recent advances include applying agent-based models to marine ecosystems (CITATION)

The marine ecosystems are inherently complex, so the trajectory of model development has understandably been in that direction. In running to more complex descriptions the model formulations have been mostly adapted from earlier models, some of which have been criticised as not making physiological sense \citep{Smith2014}.
...
So models are getting more and more complex, although basic questions about ecosystem dynamics remain unanswered. 


%- Current issues (Running before we can walk, etc.)
...
%- A lot of problems reside with the tools we use for modeling (old, inflexible, complicated)
%- Data analysis, modern workflows , modeling tools can evolve in similar direction
Often creating a model is complicated enough that scientists do not have time to focus on model to data comparisons that are generally technically challenging and time consuming.
Creating models should be easy, so that scientist can focus on scientific questions instead of being held back by technical details and code bugs.
...
- "the growing development of these models often leads to "monolithic" codes and i/o interfaces of increasing complexity, raising issues of maintainability and error-proneness."
...
"easy to use, open source, easy to modify, provide a toolbox"


% - there are modeling tools, and open source frameworks existing and being developed ("the field")
...
There are these.. and these projects (need some examples: Ecosym (written in Microsoft .NET -> hard to integrate in modern worflows), Stella modeling tool (GUI, proprietary software = not good!), etc.).
...
% - why is Phydra/XSO new, cool, and better?
...
Python can bridge the gap between older, but more efficient ecosystem model code written in low-level languages like C++ and FORTRAN and the flexibility and usability of modern high-level programming languages. Python is a free and open-source language that is flexible, popular and has gained wide use in the scientific community.
...
%- This is why, we developed Phydra / XSO
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1st paragraph: Importance of marine ecosystems and phytoplankton to global biogeochemical cycles + diversity of organisms and species

%  \subsection{Phytoplankton modeling: From NPZD to DARWIN}
% 2nd paragraph: Development and current state of marine ecosystem models

% 3rd paragraph: Current issues in the field (model complexity not tested, reproducibility)

% 4th paragraph: Part of the problem: the METHODS!
...
% inspired by VEROS introduction:
...
% 1st paragraph: what can solve the issues posed in previous paragraph?
...
% 2nd paragraph: existing modeling frameworks in marine ecosystem modeling
% 3rd paragraph: Why python? 
Why python? -> most popular programming language, rich scientific user community, Pangeo, rising language, relatively easy to learn
...
%
% 4th paragraph: present Fastscape/Xarray-simlab design and how it inspired Phydra/XSO
%\subsection{Aims}
% AIM for phydra:
