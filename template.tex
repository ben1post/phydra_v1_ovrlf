...
The XSO framework provides the building blocks for computational model based on ordinary differential equations. These building blocks are leveraged in Phydra to build a library of marine ecosystem models. The XSO framework allows for flexible modification of the structure, the components, the dimensionality and parameterisation of a model. The aim is to provide usability in line with popular Python data analysis and visualization packages (such as Pandas, Xarray and Matplotlib) for the purpose of ecological modeling. In addition, the XSO framework leverages these packages and provides direct interoperability for an integrated modeling environment.
...
\begin{itemize}
    ...
    \item  \textit{Object}: What is an object?
    
    \item  \textit{Component}: What is a component?
    
    \item  \textit{Decorator}: What is a decorator?
    
    \item  \textit{Method}: What is a method?
    
    \item  \textit{Attribute}: What is an attribute?
    
    \item  \textit{Argument}: What is an argument?
\end{itemize}
...
Xarray-simlab-ODE (XSO) provides a framework to build and customize models based on differential equations from flexible components. Phydra is an open-source Python library that contains marine ecosystem models built in a modular fashion using the XSO framework. The XSO framework was developed in conjunction with Phydra, but is domain-agnostic and can be used to build any type of model based on differential equations. The Phydra package provides a specific library for marine ecosystem modelling applications that establishes conventions and common usage. The goal is to provide a flexible framework for marine ecosystem modelling in Python that fosters creative experimentation and open exchange between scientists.

The structure and functionality of Phydra and XSO and a simplified model development workflow are presented in Figure \ref{Figure:PhydraXSOPackageSchematics} and explained in more detail below.

\subsection{The XSO framework} \label{Section:XSOFramework}
The XSO framework is an extension of the Xarray-simlab package \citep{Bovy2018Xarray-simlab:Interactively}. Xarray-simlab provides a generic modeling framework, which XSO extends with functional building blocks and a solver backend for differential equation based models. It inherits the data structure, storing model input and output as Xarray data sets, which allows for easy sharing or further analysis within the Python scientific ecosystem \citep{Hoyer2017Xarray:Python}.
...
This allows users to easily document the full model development workflow and exchange and interact at multiple stages of the modelling process. XSO provides an interface for iterative modifications, both to more complex and simpler model constructs. The typical steps of a model development are presented in further detail in section \ref{Section:ModelDevelopmentWorkflow}.
