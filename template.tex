...
We start model construction from the list of state variables, forcings and fluxes mentioned before. To add state variables, we can simply use the \texttt{StateVariable} component, which defines a single variable. The name of the specific component is supplied at model creation, in this case we have one state variable for the nutrient and one for phytoplankton. The specific label that is used to link components with each other (e.g. \texttt{"N"} for nutrient) are supplied at model setup. 
A XSO forcing variable requires a setup function that supplies the forcing to the framework. For a constant forcing we can use the supplied \texttt{ConstantForcing} component, which takes a parameter for the value of the forcing and returns a function simply returning this value at every evaluated time step. We link the created constant forcing to the \texttt{Inflow} component via its label \texttt{"N0"}. The \texttt{Inflow} component defines a rate parameter and via the supplied nutrient label \texttt{"N"} the value of \texttt{"N0"} is multiplied by the rate parameter and added to the nutrient state variable. Similarly, nutrient uptake and phytoplankton growth is defined via a single component, that takes as input the labels of the resource and consumer, as well as a maximum growth rate parameter and the half-saturation constant. 
...
$\mu^\emptyset$ & maximum growth rate & 1 & \unit{d^{-1}} \\
...
For the first use case we have implemented a traditional NPZD ecosystem model as presented in Figure \ref{Figure:ModelSchematics_2} with a nutrient $N$ (in this case nitrate), phytoplankton $P$, zooplankton $Z$ and detritus $D$ as state variables. The first use case model employs slab physics as presented in \citet{Evans1985ACycles}. The model ocean is built up of two layers. A biologically inert deep ocean is situated below a well mixed upper layer of variable depth that contains the ecosystem. The model structure is adapted from the EMPOWER model presented by \citet{Anderson2015c}.

The model is driven by empirical forcing describing the depth of the mixed layer ($H$), average temperature of the mixed layer ($T$), photosynthetically active radiation at the surface ($I$) and nutrient concentration in the deep layer ($N^\emptyset$) 
...
Phytoplankton biomass ($P$) increases through temperature-dependent, light- \& nutrient-limited growth. The growth rate ($\mu_{P}$) is the product of a temperature-dependent maximum growth rate ($\mu_P^{max}(T)$) and the growth-dependencies on light ($\gamma_{I}$) and nutrients ($\gamma_{N}$): 

\begin{equation}
    \mu_{P} = \mu_P^{max}(T) \ \gamma_{I} \ \gamma_{N}
\end{equation}

$T$ is the average temperature of the mixed layer in \unit{\degree C}, as supplied from model forcing. The maximum growth rate of phytoplankton $\mu_P^{max}(T)$ is equivalent to the temperature-dependent maximum photosynthetic rate $V_P^{max}(T)$. Temperature dependence is calculated via the Eppley curve \citep{Eppley1972TemperatureSea}, with an exponential equivalent to a $Q_{10}$ of 1.895.

\begin{equation}
    \gamma^{T} = V_P^{max}(T) \ 1.066^T
\end{equation}
...
% Perhaps show full equation in appendix?
...
See \citet{Anderson2015c} for further discussion of the used formulations and a more in-depth presentation light-limitation in slab models.
...
\begin{equation}
    G_P = \mu_Z \left( \frac{ \hat{\phi}_P P}{k_Z^2 + \hat{\phi}_D D +\hat{\phi}_P P}  \right) Z
\end{equation}
where $\hat{\phi}_P$ = $\phi_P \ P$, $\hat{\phi}_D$ = $\phi_D \ D$.
...
The grazing preference parameters $\phi_P$ and $\phi_D$ do not represent a discrete fraction of the amount grazed in the diet relative to the environment. Instead, this amount is represented by the ratio of $\hat{\phi}_P$ and $\hat{\phi}_D$. 
The half-saturation constant for grazing $k_Z$ is similarly an arbitrary parameter, that scales the density-dependent half-saturation constant $k_P$ for grazing on phytoplankton based on the choice of $\phi_P$, with the relationship $k_P$ = $\sqrt{\frac{k_Z^2 }{ \phi_P}}$.


Similarly the detritus grazing flux is defined as:

\begin{equation}
    G_D = \mu_Z \left( \frac{ \hat{\phi}_D D}{k_Z^2 + \hat{\phi}_D D +\hat{\phi}_P P}  \right) Z
\end{equation}
...
need to edit this to large table, for all models
...
$\phi_P$ & grazing preference: P & 0.67 & 0.67 & 0.67 & 0.67 & \\
$\phi_D$ & grazing preference: D & 0.33 & 0.33 & 0.33 & 0.33 & \\
