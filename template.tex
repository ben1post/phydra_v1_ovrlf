...
\Author[2]{Esteban}{Acevedo-Trejos}
\Author[3]{Andrew }{Barton}
\Author[2]{Benoît}{Bovy}
\Author[1]{Agostino}{Merico}



\affil[1]{Leibniz Centre for Tropical Marine Research (ZMT), Bremen, Germany}
\affil[2]{GFZ German Research Centre for Geosciences, Potsdam, Germany}
\affil[3]{Scripps Institution of Oceanography, La Jolla, CA, United States}
...
The XSO framework and Phydra library aim at empowering scientists to do better research in less time, collaborate efficiently and make new discoveries. Our goal is to make the technical implementation of writing functions and setting up a model more accessible, so that the focus can be on scientific questions and creating the most appropriate model structure and parameterization.

In the next section, we present the usage of the XSO framework and structure of the Phydra library, including the steps of an exemplary model development workflow. In the following model use case section, we show the utility of the tool-set in three exemplary model applications: A basic NP chemostat model, an NPZD model in a slab physical setting adapted from \citet{Anderson2015c} and a complex size-structured model in a simple box setting adapted from \citet{Banas2011b}. We then discuss the framework architecture, current limitations and possible future developments of XSO and Phydra and finally conclude the study.
...
The open-source code allows users to easily develop or modify processes to describe a specific ecosystem. Users of Phydra are very welcome to provide their own code to the standard library in a collaborative effort to support efficient, open and easily reproducible marine ecosystem model development. Phydra can thus provide a robust, documented and peer-reviewed code basis for the scientific exploration of marine ecosystem models.
...
\caption{Model schematic for use case 1: Monod model of phytoplankton feeding on a single nutrient in a flow-through system. The light blue shape represents the model system, with an external nutrient medium with concentration $N_0$ flowing into $N$, which is in turn taken up by $P$. Both $N$ and $P$ flow out at a constant rate and the modeled volume remains constant.}
...
To explore the basic dynamics, we chose parameters arbitrarily from standard ranges for exemplary model runs. See table \ref{Table:UseCase1Parameters} for the specific parameters values used. Initial values for the state variables $N$ and $P$ were set at 1 and 0.1 respectively. A more profound analysis would require testing parameter ranges, but for the simple demonstration we aim at here this should suffice.
...
Dissolved inorganic nitrogen in the mixed layer ($N$) is supplied via mixing, zooplankton excretion and detritus remineralization.
...
Detritus concentration in the upper layer ($D$) is supplied by all mortality of phytoplankton, linear zooplankton mortality and zooplankton egestion (e.g. fecal pellets). The loss terms are remineralization, zooplankton grazing, mixing and an additional sinking flux. 

Detritus is remineralized at a constant rate $m_D$ to $N$. Similar to $P$ and $Z$, $D$ is affected by mixing through changes in MLD, described by the mixing coefficient $K$. In addition to $K$, detritus experiences losses due to gravitational sinking at a rate of $v_D$. This term is added to describe the fast export of larger detritus particles below the mixed layer. 
...
\caption{Schematic of use case 2 model as implemented in the XSO framework and included in the Phydra library. To simplify visualization, only the XSO components with their labels and linkage are shown. Each component consist of a number of variables, forcings or parameters. Solid arrows indicate the flow of a flux between state variables. Dashed arrows visualize flux values passed along as group variables. Dashed lines connecting processes indicate variables and forcings passed along via their label.}
\label{Figure:CodeSchematics_2}
\end{figure*}

Similarly to the chemostat model, we implemented the NPZD slab model using the XSO framework, where we separated the model into state variables, forcings and fluxes. For state variables we have nutrient ($N$), phytoplankton ($P$), zooplankton ($Z$) and detritus ($D$). Forcings to the model are the mixed layer depth ($H$), nutrient concentration below the mixed layer ($N_0$), temperature in the mixed layer ($T$) and irradiance at surface ($I$). The model defines at least ten unique fluxes: Phytoplankton growth, zooplankton grazing, nutrient upwelling, mixing, sinking, remineralization, in addition to four mortality terms.
...
\caption{Schematic of use case 3 model as implemented in the XSO framework and included in the Phydra library. To simplify visualization, only the XSO components with their labels and linkage are shown. Each component consist of a number of variables, forcings or parameters. Solid arrows indicate the flow of a flux between state variables. Dashed arrows visualize flux values passed along as group variables. Dashed lines connecting processes indicate variables and forcings passed along via their label. Arrows with multiple lines indicate values with dimensions that are passed along.}
...
The implementation of the GEKKO solver backend does not yet provide access to the powerful features for model optimization provided by GEKKO. It was included in the first release as a proof-of-concept to showcase the compatibility with advanced solver algorithms. As the XSO framework is developed we are looking to extend the framework interface to allow users to directly access the additional functionality of GEKKO.
...
The Xarray-simlab package that provides the basis for the XSO framework is a relatively young project and actively under development, which provides some challenges, but also allows for many future improvements to the functionality of Phydra and XSO. The Phydra library does not have to be restrained to the XSO framework in future development. The Phydra library could be expanded to provide a functional Python interface to backend code specifically suited to marine ecosystem model applications (e.g. legacy Fortran code).
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \texttt{list\_input}: XSO implements this feature to allow greater flexibility in setting up components that act on multiple variables similarly. The list input feature can be activated via \texttt{list\_input = True} if the \textit{state variable} is defined as \texttt{foreign = True}. At model setup the user can supply a list of labels that are iterated over, calculating the specific flux acting on each supplied \textit{state variable}.
...
\begin{enumerate}
    ...
    \item Detritus remineralization:
    \begin{itemize}
...
$m_D$ & D remineralization rate & 0.06 & 0.06 & 0.06 & 0.06 & \unit{d^{−1}} \\
