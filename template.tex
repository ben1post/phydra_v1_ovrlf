...
% this needs to be more user workflow, less backend explanation (leave that to previous framework explanation), instead focus on the actual functions and arguments, step by step
    ...
    To create a custom model, the function \texttt{xso.model()} is called with a single argument:
    \begin{itemize}
        \item \texttt{model_dict}: A dictionary of model \textit{components} with their respective labels. The supplied labels identify the \textit{components} for all future steps and allow reusing \textit{components} with unique labels.
    \end{itemize}
    ...
    - In order to fully initialize a model that can be solved, the model object needs to be supplied with a complete set of parameters, labels linking forcings and variables between \textit{components}, as well as the time steps and solving algorithm to use.
    ...
    \begin{itemize}
        \item \texttt{model}: \textit{model object}
        \item \texttt{solver}: label of built in solver or class of custom solver
        \item \texttt{input_vars}: a dictionary of input parameters
        Multiple values for a specific parameter can be passed with an additional batch dimension. At model runtime the model can be solved for each set of parameters within the batch dimension.
        
        \item \texttt{output_vars}: The user can specify output variables, by default (if no input is provided) all values of variables, forcings and fluxes are stored after model runtime.
    ...
    The runtime method is provided by the underlying Xarray-simlab framework, of which the following arguments are currently supported by XSO:
    \begin{itemize}
        \item \texttt{model}: \textit{model object}
        ...
        \item \texttt{store}: This optional argument allows specifying a string path where simulation input and output data is stored. If the argument is not specified, all data will be saved in memory.
    \end{itemize}
    A "filled-out" dataset is returned containing model setup parameters and model output in labelled dimensions. The model output is again a Xarray dataset, fully compatible with all methods and extensions for storing, analyzing or visualizing the data provided by Xarray.
    
    \item \textbf{Storing model output}: 
    - Typically a user would assign the model output to a new variable in Python.
    - The model output is a Xarray dataset, that provides a wealth of functionality for storing the specific dataset. We want to highlight the \texttt{to_netcdf()} method that stores the full model output including metadata and labeled dimensions to an NetCDF file at the user-specified location.
    
    XSO provides several options to store model output, as well as storing the value of fluxes and forcings. This allows users to specify the model output that suits the complexity of the model construct.
    ...
    - model output data can be extracted by labels (how?)
...
Phydra comes with a library of \textit{components} that were created using the XSO framework. To implement models beyond those included in the Phydra library, users can build custom \textit{components} from a set \textit{variable types} provided by the XSO framework. \textit{Components} are simply Python classes with the \textit{variable types} as attributes and methods that are decorated with \texttt{@xso.component}. The decorating function registers the \textit{variable types} within the framework, greatly reducing unnecessary boilerplate code and creates fully functional model building blocks (i.e. Xarray-simlab \textit{processes}). Existing \textit{components} can be easily modified to have a different functionality, as long as the dimensions of the included state variables and fluxes match those of the interacting components in the model. 
...
The currently available \textit{variable types} in the XSO framework are explained below:
\begin{itemize}
    ...
    - State variables can be defined as unique instances, but also as functional groups (essentially an array of state variables). Dimensions need to be defined via custom string labels supplied to the \texttt{dims} argument.
    ...
    - This string label is also supplied to Name argument of the \textit{group} variable, and all variable defined in other components that match the label are collected as a list. 
