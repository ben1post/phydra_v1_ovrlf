...
\begin{enumerate}
    ...
    \item \texttt{MonodComponent} is another component that defines a flux, in this case the constant input of medium with nutrient concentration $N_0$ into our medium.
    \begin{itemize}
        \item \texttt{resource}: A \texttt{xso.variable} with the argument \texttt{foreign=True}. 
        \item \texttt{forcing}: A \texttt{xso.forcing} with the argument \texttt{foreign=True}. 
        \item \texttt{rate}: A \texttt{xso.parameter} whose value is supplied at model setup and is used within the flux function below.
        \item \texttt{def inflow(*)}: This is 
    \end{itemize}
    \item Phytoplankton growth
    
    \item Outflux
\end{enumerate}


The nutrient concentration in the external medium is supplied from the \texttt{ConstantForcing} component.A XSO forcing variable requires a setup function that supplies the forcing to the framework. For a constant forcing we can use the supplied \texttt{ConstantForcing} component, which takes a parameter for the value of the forcing and returns a function simply returning this value at every evaluated time step. We link the created constant forcing to the \texttt{Inflow} component via its label \texttt{"N0"}.

The \texttt{Inflow} component defines a rate parameter and via the supplied nutrient label \texttt{"N"} the value of \texttt{"N0"} is multiplied by the rate parameter and added to the nutrient state variable. Similarly, nutrient uptake and phytoplankton growth is defined via a single component, that takes as input the labels of the resource and consumer, as well as a maximum growth rate parameter and the half-saturation constant. 
...
The amount of fluxes and interdependencies between the calculations require a more complex component structure than our first presented use case. The user needs to strike a balance between component refactoring and simplicity. Our goal was to allow for every individual ecologically relevant term to be exchangeable, for example each growth-limiting term was defined via an individual component. The "group variable" feature of the XSO framework allows for such a setup, so that as long as another flux defined in a component matches the group label in another component, it will be used for calculating the bulk flux defined there. The group variable can collect multiple fluxes to sum or multiply over, but it can also be used to split the calculation of a single flux between two components. This feature was used frequently to handle the complexity of the presented model, for further detail see the description below.

- The modular nature of XSO allows reusing calculated terms in different fluxes using the group variable feature. 
...
\begin{itemize}
    ...
    \item The calculation of mixing in this slab model takes the value of $H$ from the \texttt{MLDForcing} component, where the forcing is read from a file, interpolated and supplied via the label \texttt{"MLD"} to the framework. This forcing is supplied to the \texttt{Mixing\_K} component, where the mixing coefficient $K$ is calculated once and supplied to \texttt{SlabUpwelling} and \texttt{SlabMixing} via a group variable. This allows calculating $K$ only once per time-step for more efficient computation. \texttt{SlabUpwelling} takes calculates nutrient upwelling via the value of $K$ and the difference between the $N_0$ forcing and $N$. \texttt{SlabMixing} employs the list-input functionality described in the first use case, and calculates the effect of mixing on $P$, $Z$ and $D$. Finally, \texttt{SlabMixing} also receives the MLD forcing ($H$) from \texttt{MLDForcing} and calculates the sinking flux acting on $D$.
    
    \item Temperature-dependent, light- and nutrient-limited growth of phytoplankton is another model process that was implemented with multiple linked components, to allow for greater flexibility. \texttt{GrowthML} is the process that receives all the separate terms and calculates the net growth flux from a group variable. That group variable is added to from both the \texttt{SmithLightLim} component, calculating the integrated light limitation based on the maximum photosynthetic rate passed from \texttt{TempDepGrowth}, as well as the \texttt{MonodGrowthLim} component that calculates nutrient-limitation based on $N$. The forcing supplying irradiance at surface ($I$) is passed from the \texttt{PARForcing} component to \texttt{SmithLightLim}, similarly the temperature forcing ($T$) is supplied via the \texttt{SSTForcing} component to \texttt{TempDepGrowth}.
    
    \item The grazing flux is set up via two components, that are again linked via a group variable. Grazing is split into \texttt{HollingType3}, where the total grazing fluxes are calculated that subtract from $P$ and $D$, and \texttt{GrowthGrowthEfficiency}, which handles the routing of the total grazed biomass split into three fractions, which are $P$ assimilated to $Z$, egested to $D$ and excreted to $N$). To simplify the model setup, \texttt{HollingType3} was implemented with a list input for the resource variable, so that it would be simple to add or remove state variables from the zooplankton prey pool. The grazing preference is supplied as a list of the same dimension, where the order of values has to correspond to the order of supplied labels for the resource variable.
    
    \item Remineralisation is implemented using the component \texttt{LinearExchange}, which takes a source and sink variable as input, as well as a rate parameter ($m_D$ in this case).
    
    \item Mortality fluxes can similarly be implemented using basic components from the Phydra library. In fact we reuse the \texttt{LinearExchange} component for the linear mortality terms of $P$ and $Z$, both feeding into $D$. Quadratic phytoplankton mortality was implemented with the \texttt{QuadExchange} component, as it also feeds into $D$. Higher-order mortality of zooplankton is lost from the system and is implemented using the \texttt{QuadDecay} component.
\end{itemize}

- For the ecological description we followed the EMPOWER model structure, however the technical implementation using the XSO framework is quite different. The XSO framework defines functions irrespective of the specific time-step used for evaluation. This requires writing the functions without explicit time-steps. We appreciate the choice of transparently including the solving algorithm in the model code, however the present project aims at maximum flexibility. Different packaged and even custom coded model solving algorithms can be implemented via the backend Solver class by more advanced users.
...
The forcing used in the presented model runs was adapted from 
...
- talk about the problem with linearly interpolated forcing, as used in original EMPOWER
- careful choice is necessary, model already calculates aggregate properties, both in spatial and temporal direction. Marked dynamics of model should not be an artifact of forcing interpolation method.
...
%t
\begin{table*}[t]
\caption{Model parameters used in runs for use case 2: TODO: add all 4 locations}
...
- showcase "batch" execution, can run all 4 location models from single model setup. Output is stored as one Xarray, with additional dimension coordinate that differentiates between locations.. allows easy analysis of parameter scans and bulk analysis of complicated model runs.
...
Non-grazing mortality of phytoplankton is described the factor $m^P$ that is scaled by the maximum intrinsic growth rate $\mu_{max}^i$. This accounts for natural mortality and excretion.
...
\begin{equation}
    \frac{d N}{d t} = 
    f \ N_0 % Nutrient mixing
    +  \beta (1 - \epsilon) \sum_{j} \sum_{i} G_P^{ij} % Unassimilated grazing by Z
...
\begin{equation}
    \frac{d Z_j}{d t} =
    \beta \ \epsilon \ \sum_{i} G_P^{ij} % Assimilated grazing
