...
GENERAL STRUCTURE:
\begin{enumerate}
    \item Components: XSO model component library
    
    
    \item Models: Model objects constructed from component library
    
    
    \item Example Notebooks: Exemplary model runs, incl. parameters and plots, using model library
...
- So another reason for having Phydra as a separate library, is to establish a common usage, e.g. for dimensions naming and component refactoring, so that developed models are automatically compatible with each other, to foster creative experimentation.

The implementation and testing of more complex ecosystem descriptions is the natural application of such a package, which is demonstrated in the three use cases presented in Section \ref{Section:UseCases}. 
...
The object-oriented structure of the XSO framework allows users to extend certain base processes to provide the desired functionality. 
...
% XSO backend
- explain XSO Backend, Time, Solver, ...
