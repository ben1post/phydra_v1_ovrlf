...
- Achieving flexibility of modeling frameworks in a practical software implementation is challenging.
- Frameworks generally require a level of abstraction.
- Trade-off between frameworks with high level of generality and abstraction that are easier to use but limited in application and frameworks that operate at very low level and require high effort to learn and use. A framework that over-restricts the user interface may be easy to manage, but reduces possible applications. 

- Situated in complexity between GUI tools that allow drag and drop, and custom model scripts. 
- XSO as a framework aims to reduce the boilerplate code necessary to write computational models in Python. Phydra as a library provides a reviewed code base that users can start from.

% framework with Python backend and Python frontend
- Frameworks can be implemented using a custom scripting language or provide pre-built model objects that can be extended through inheritance.
- The XSO framework aims to strike a balance between these two levels. Python is very popular and widely used by scientists. Basic syntax of XSO should be easily accessible by users proficient in Python. Instead of requiring a custom scripting language (e.g. YAML), the provided \textit{variable types} and \textit{component} decorators supply a simplified user interface that is rooted in basic Python functionality. 
- level of abstraction between \textit{variable types} and \textit{components}, backend technical implementation can be updated and improved without breaking functionality of existing model \textit{components} and models.

- This design however reduces accessibility to scientist not proficient in Python, might require a steeper learning curve.
- another benefit: XSO framework does generally not restrict packages, syntax or math used in defined functions. The underlying Xarray-simlab goes beyond that in flexibility, so that we had to define our own custom extension to simplify setting up ODE-based models.
- Since XSO mostly wraps Xarray-simlab, this framework is available to users for further extension and custom functionality
- allows user to use their own packages or functionality within the Python syntax

% flexibility in model construction
- Conceptual ecosystem models vary greatly in complexity. 
...
- \textit{Component} bundles certain functions, parameters and variables.
- \textit{Components} can theoretically contain any number of \textit{variable types}, but this is practically limited by 
- Every aspect of model needs to defined at the \textit{variable type} level within components, no implicit model structure otherwise.. (i.e. what i want to say: XSO has no "magic" everything needs to be clearly defined in \textit{components} of the model.) BUT components that handle complex setups can be reused and shared.
- State variables, Forcings, etc. need to be set up in one \textit{component}, can be referenced across the model, system of ODEs is constructed via the \textit{fluxes} that are assigned to the referenced state variables.

- Because of framework design: Effort required to construct models is proportional to required model complexity!
...
- First version of XSO only supports building models based on ODEs.
- The XSO framework as-of-yet does not support completely different models, such as agent based models.
- no advanced features like parameter optimization yet, although technically possible
- benefits from mathematical formulations that are the same for both input and output terms of a particular \textit{flux} function. No feature yet that simplifies unit conversions, etc.

% XSO solvers implemented so far, python = slow
...
- "problem" with high-level scripted language python: inherently slow! but many ways to circumvent
- The solver backend can be extended to include solvers compiling model to machine code 
- odeint makes full use of numpy vectorization, relatively fast and efficient for presented use cases

% Current design paradigm rooted in flexible labeling to link components
- Links between model components are defined via supplied labels at model setup! This is not ideal for complex models, requiring many labels. For the presented use cases this is easily manageable. However implicit links between components is possible and should be considered for constructing models that aim at a high level of complexity, where the user does not require easy exchange of many model components.
- Current design paradigm made for rapid prototyping and easily exchanging and reusing \textit{components}
- built-in links between \textit{components} are particularly useful for canonical ecosystem models, where the main goal is to extend and experiment with a specific model setup.

% Phydra focus on simple physical settings for ecosystem models (no dimensionality yet!)
- For large spatial applications not suitable, will need to include parallelisation, but this functionality is inherently supported by the underlying Xarray-simlab architecture. Solver backend will need to be optimized to support it in the future
- presented use cases limited to zero dimensional physical settings
- number of model dimensions need to be defined within \textit{components} and needs to match within the model
...
- Conda install via \texttt{conda install conda-forge phydra}, automatically checks dependencies and installs required packages.
...
Both of Xarray-simlab and GEKKO are relatively young Python packages and actively under development, which provides some challenges, but also allows for many future improvements to the functionality of Phydra and XSO.
- In fact, backend is not fixed, and project could expand to wrap more advanced backend code specifically suited to marine ecosystem model applications (e.g. legacy Fortran code)
- These type of frameworks are not built for rock-solid long maintained performance, Python and the underlying packages are developing fast. But that bug is also a feature: fast development, very extensible and flexible
- Can be circumvented with appropriate environment manager e.g. Conda (very recommended to users of Phydra), checks dependencies and versions at installation, should allow users relatively setup
% CONDA NAME DROP
...
- ""The framework allows to build models very quickly and dynamically from many components by automating aspects such as workflow dependencies and model input detection. It has also potential to automate other aspects like parallel execution of model components, command-line interface and/or graphical interface.""
- XSO stores model equations including the names of parameters and it would be possible to add some functions that print the mathematical equations.. which would be quite helpful to build and debug a model. (possible feature: further model introspection and debugging)
- possible feature: create interface/methods for parameter optimization
- possible feature: create custom methods to expand model dimensionality, allow straight-forward definition of boundary condition and exchange between model compartments
% above and below could be merged into a single paragraph
% OPEN SOURCE CONTRIBUTION 
We hope that Phydra offers the marine ecosystem modeling community a new tool for research and practical applications. New developments are very likely to arise from such work and collaborations. 
- In particular open collaboration can greatly expand the library of \textit{components} and \textit{model objects} beyond those presented here.
Phydra and XSO are open-source projects. Contributions are welcome and in fact greatly appreciated. Users can contribute in many ways by reporting bugs, submitting feedback, contributing to the development of the code or the documentation. 
The code is fully accessible on Github with LICENSE and distributed using the Python package installer PyPI and through a channel on the package manager Conda. A guide on how interested users can contribute to the project is provided in the online documentation.
