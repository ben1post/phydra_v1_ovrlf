...
Xarray-simlab-ODE (XSO) is a Python framework that allows you to construct and customize models based on ordinary differential equations (ODEs) in a modular fashion. XSO was developed as the technical foundation of the Phydra library, but is not limited to any particular domain and can used to create ODE-based models of any type.

The XSO framework is an extension of Xarray-simlab \citep{Bovy2018Xarray-simlab:Interactively}, which provides a generic and highly flexible model development framework in Python using compact data classes and decorators. Xarray-simlab has found fruitful applications, for example in landscape evolution modeling \citep{benoit_bovy_2020_3840917} and plant growth modeling \citep{Vaillant2022TowardsDevelopment}.

Data classes and decorators are tools for object oriented programming (OOP) that highly simplify writing classes and allow wrapping functionality within custom variables in Python. XSO makes use of these same tools to extend Xarray-simlab with building blocks that can interact with a custom solver backend for ODE-based models. 

% The following is quite technical:
The backend code is structured into a \textit{core} class that provides an interface between a model backend and an extensible solver class. As an open-source project, this backend code is fully accessible and was designed to allow advanced users to extend functionality and provide custom solver implementations. Basic usage of XSO does not require any interaction with the backend, or any understanding of the OOP principles, as all functionality is wrapped in custom variables and decorators provided to construct \textit{components}. 
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{xso.variable}: Defines a state variable, either locally or via reference in another component.
        \item \texttt{xso.forcing}: Defines an external forcing as a constant or time-varying value, via an additional setup function.
        \item \texttt{xso.parameter}: Defines a constant model parameter.
        \item \texttt{xso.flux}: Defines a mathematical term in the system of differential equations.
        \item \texttt{xso.group}:
    \end{itemize}
...
It inherits the data structure from Xarray-simlab, storing model input and output as Xarray datasets \citep{Hoyer2017Xarray:Python}. XSO provides an interface for iterative modifications, both to more complex and simpler model constructs. The typical steps of a model development workflow are presented in figure \ref{Figure:PhydraXSOPackageSchematics}.



% REALLY IMPORTANT:
Highlight flexible dimensionality as core feature!


% DON'T REPEAT YOURSELF OVER AND OVER !
% Mention limitiation
% But highlight the flexibility and benefits
The XSO framework is currently limited to model applications based on ordinary differential equations (ODEs). With this limitation, conceptual marine ecosystem models still vary greatly in complexity. Our objective in developing the XSO framework was to enable users to construct ODE-based models of any complexity, particularly in regards to the number of state variables and processes involved. 
...
\begin{enumerate}
    ...
    \item \textbf{\textit{Model objects}}: The first release of phydra contains the \textit{model objects} defined in the three model applications presented in section \ref{Section:UseCases}. The \textit{model objects} can be imported from the library and can be setup, modified, and run by a user.
    
    \item \textbf{Example notebooks}: \textit{Model objects} only define the collection of \textit{components}. To run a model, the input parameters still need to be defined and supplied at runtime. The Phydra library comes with three fully documented model applications that are presented in Jupyter notebooks. These notebooks show all steps from creating the \textit{model setup} object to analysing model output and provide a template for further exploration and experimentation with the provided marine ecosystem models.
